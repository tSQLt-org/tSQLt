...
BEGIN
  ...
  DECLARE @object_id INT;SET @object_id = OBJECT_ID('Private_RemoveSchemaBindingTests.V1');
...
END;
GO
CREATE PROCEDURE Private_RemoveSchemaBindingTests.[test works on indexed view]
AS
BEGIN
  CREATE TABLE Private_RemoveSchemaBindingTests.T1(C1 INT);
  EXEC('CREATE VIEW Private_RemoveSchemaBindingTests.V1 WITH SCHEMABINDING AS SELECT T.C1,''A Constant'' AS C2 FROM Private_RemoveSchemaBindingTests.T1 AS T;');
  EXEC('CREATE UNIQUE CLUSTERED INDEX [CI:V1] ON Private_RemoveSchemaBindingTests.V1(C1);');
  EXEC('CREATE NONCLUSTERED INDEX [NCI:V1] ON Private_RemoveSchemaBindingTests.V1(C2);');
  INSERT INTO Private_RemoveSchemaBindingTests.T1(C1)
  VALUES(42);
